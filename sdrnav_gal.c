...
extern int decode_e1b(sdrnav_t *nav)
{
    ...
    /* poly[2]={0x4f,0x6d} = {G2,G1}: i%2==0 is G2 (inverted at TX), i%2==1 is G1 */
    for (i=0;i<240;i++) {
        if (i%2==0)
            enc_e1b[i]=(bits_e1b[i]==1)? 0:255;  /* G2 - undo TX inversion */
        else
            enc_e1b[i]=(bits_e1b[i]==1)? 255:0;  /* G1 - no inversion */
    }
