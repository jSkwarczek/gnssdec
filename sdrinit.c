...
extern int initnavstruct(int sys, int ctype, int prn, sdrnav_t *nav) {
    ...
    /* GNSS-SDRLIB uses {0x4f, 0x6d} = {G2, G1} order, but system libfec
     * has V27POLYA=0x6d and V27POLYB=0x4f (swapped).
     * Use explicit values for compatibility with Galileo soft-decision mapping */
    int poly[2] = {0x4f, 0x6d};  /* G2, G1 order - matches GNSS-SDRLIB convention */
